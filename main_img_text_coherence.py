...
# Dataset settings
NUM_SAMPLES = 20  # Number of image pairs to generate
OUTPUT_DIR = "data_complex2"  # Subfolder where dataset will be saved
...
LLM_API_KEY = None  # Set to your API key, or None to read from OPENAI_API_KEY environment variable
...
PRESENTATION_TEXT_MAX_LENGTH = 200  # Maximum length of presentation text in characters
# Note: For OpenAI, uses GPT-4o vision model - set your API key: export OPENAI_API_KEY="your-key-here"
#       For Ollama, uses vision models if available (e.g., llama3.2-vision)
#       Vision models analyze the actual image to generate presentation text
...
def generate_meaningful_title():
    """Generates meaningful presentation slide titles using LLM or fallback."""
    return llm_generator.generate_title()

def generate_meaningful_bullet_point():
    """Generates meaningful bullet points for presentations using LLM or fallback."""
    return llm_generator.generate_bullet_point()


def generate_meaningful_table_headers(num_cols):
    """Generates meaningful table headers based on number of columns using LLM or fallback."""
    return llm_generator.generate_table_headers(num_cols)


def generate_meaningful_table_data(num_rows, num_cols, headers):
    """Generates meaningful table data based on headers."""
    data = []
    
    for row in range(num_rows):
        row_data = []
        for col_idx, header in enumerate(headers):
            # Use LLM generator's table cell data method
...
def create_complex_images(dataset_directory, num_samples=50, generate_image=None):
    ...
    Returns list of tuples: (image_path, presentation_text, slide_data)
    """
    images = []
    presentation_texts = []
    full_data = []
    ...
    while i < num_samples:
        ...
        # Generate larger table data with meaningful content for 224x224 images
        ...
        table_headers = generate_meaningful_table_headers(num_cols)
        table_data = generate_meaningful_table_data(num_rows, num_cols, table_headers)
        ...
        if GENERATE_PRESENTATION_TEXT:
            try:
                ...
                print(f"Generated presentation text for img1_{i}.png: {presentation_text}")
            except Exception as e:
                print(f"Warning: Failed to generate presentation text from image: {e}")
                presentation_text = f"Presentation about this slide is not possible(image analysis failed)"
        else:
